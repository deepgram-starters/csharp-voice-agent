...
C# (.NET) demo app for Deepgram Voice Agent.

## Architecture

- **Backend:** C# (.NET) (C#) on port 8081
...
**Clean rebuild:**
```bash
rm -rf bin obj frontend/node_modules frontend/.vite
make init
```
...
```
feat(csharp-voice-agent): add diarization support
fix(csharp-voice-agent): resolve WebSocket close handling
refactor(csharp-voice-agent): simplify session endpoint
